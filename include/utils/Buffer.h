...
class Buffer {
 ...
 public:
  ...
  [[nodiscard]] inline int64_t readInt64(const uint8_t* buffer,
                                         size_t offset) const noexcept {
    if (endian() == Endian::kLittleEndian) {
  ...
  inline void write(uint8_t* buffer, const char* value, size_t size,
                    size_t offset) const noexcept {
    memcpy(buffer + offset, value, size);
  }

  inline void write(uint8_t* buffer, const std::string& value, size_t size,
                    size_t offset) const noexcept {
    write(buffer, value.c_str(), size, offset);
  }

  inline void write(uint8_t* buffer, const char* value,
                    size_t offset) const noexcept {
    write(buffer, value, strlen(value), offset);
  }

  inline void write(uint8_t* buffer, const std::string& value,
                    size_t offset) const noexcept {
    write(buffer, value, strlen(value.c_str()), offset);
  }
};
